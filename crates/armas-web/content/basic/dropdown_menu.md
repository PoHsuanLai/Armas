...
let mut menu = DropdownMenu::new("my_menu").open(is_open);
...
let mut menu = DropdownMenu::new("shortcuts").open(is_open);
let response = menu.show(ui.ctx(), button_response.rect, |menu| {
...
if button_response.clicked() { is_open = !is_open; }
let mut menu = DropdownMenu::new("checkbox").open(is_open);
let response = menu.show(ui.ctx(), button_response.rect, |menu| {
...
let button_response = Button::new("File").show(ui);
if button_response.clicked() { is_open = !is_open; }
let mut menu = DropdownMenu::new("submenu").open(is_open);
let response = menu.show(ui.ctx(), button_response.rect, |menu| {
    menu.item("New");
...
if button_response.clicked() { is_open = !is_open; }
let mut menu = DropdownMenu::new("destructive").open(is_open);
let response = menu.show(ui.ctx(), button_response.rect, |menu| {
    menu.item("Edit");
    menu.item("Duplicate");
    menu.separator();
