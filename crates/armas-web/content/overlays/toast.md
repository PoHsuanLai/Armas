...
if Button::new("Show Toast").show(ui, &theme).clicked() {
...
ui.horizontal(|ui| {
    ...
    if Button::new("Default").show(ui, &theme).clicked() {
        toasts.toast("This is a notification");
    }
    if Button::new("Error").show(ui, &theme).clicked() {
...
ui.horizontal(|ui| {
    ...
    if Button::new("Top Right").show(ui, &theme).clicked() {
    ...
    if Button::new("Bottom Right").show(ui, &theme).clicked() {
...
let mut toasts: ToastManager = ui.ctx().data_mut(|d| d.get_temp(state_id).unwrap_or_else(ToastManager::new));
if Button::new("Show With Title").show(ui, &theme).clicked() {
...
let mut toasts: ToastManager = ui.ctx().data_mut(|d| d.get_temp(state_id).unwrap_or_else(ToastManager::new));
if Button::new("10 Second Toast").show(ui, &theme).clicked() {
