...
if Button::new("Open Sheet").show(ui, &theme).clicked() {
    is_open = true;
...
let response = sheet.show(ui.ctx(), &theme, |ui| {
    if is_open {
        ui.horizontal(|ui| {
            ...
            Toggle::new().id("sheet_dark_mode").size(ToggleSize::Medium).show(ui, &mut dark_mode, &theme);
            ui.label("Dark Mode");
        ...
        ui.horizontal(|ui| {
            ...
            Toggle::new().id("sheet_notifications").size(ToggleSize::Medium).show(ui, &mut notifications, &theme);
...
let mut is_open = ui.data_mut(|d| d.get_temp::<bool>(id).unwrap_or(false));
if Button::new("Open Left Sheet").show(ui, &theme).clicked() {
    is_open = true;
    ui.data_mut(|d| d.insert_temp(id, is_open));
}
...
let response = sheet.show(ui.ctx(), &theme, |ui| {
    if is_open {
        Button::new("Home").variant(ButtonVariant::Ghost).full_width(true).show(ui, &theme);
        Button::new("Dashboard").variant(ButtonVariant::Ghost).full_width(true).show(ui, &theme);
        Button::new("Settings").variant(ButtonVariant::Ghost).full_width(true).show(ui, &theme);
    }
});
if response.closed {
    is_open = false;
