...
let response = sheet.show(ui.ctx(), &theme, |ui| {
    if is_open {
        ui.horizontal(|ui| {
            ...
            Switch::new().id("sheet_dark_mode").size(SwitchSize::Medium).show(ui, &mut dark_mode);
            ui.label("Dark Mode");
        ...
        ui.horizontal(|ui| {
            ...
            Switch::new().id("sheet_notifications").size(SwitchSize::Medium).show(ui, &mut notifications);
