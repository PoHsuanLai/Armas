...
impl TiltCard {
    ...
    pub fn new(width: f32, height: f32, theme: &Theme) -> Self {
        ...
        Self {
            ...
            tilt_x_spring: SpringAnimation::new(0.0, 0.0).with_params(150.0, 15.0),
            tilt_y_spring: SpringAnimation::new(0.0, 0.0).with_params(150.0, 15.0),
        }
    }

    /// Set tilt strength (0.0 to 1.0)
    pub fn with_tilt_strength(mut self, strength: f32) -> Self {
        self.tilt_strength = strength.clamp(0.0, 1.0);
        self
    }

    /// Enable/disable glare effect
    pub fn with_glare(mut self, enabled: bool) -> Self {
    ...
    /// Set border color
    pub fn with_border(mut self, color: Color32) -> Self {
    ...
    pub fn show<R>(
        ...
        painter.rect_filled(tilted_rect, self.corner_radius, self.background);
        ...
        if self.glare_enabled && self.mouse_pos.is_some() {
            ...
            if glare_strength > 0.01 {
                ...
                painter.circle_filled(
                    glare_center,
                    100.0,
                    Color32::from_rgba_unmultiplied(255, 255, 255, (glare_strength * 50.0) as u8),
                );
                painter.circle_filled(
                    glare_center,
                    60.0,
                    Color32::from_rgba_unmultiplied(255, 255, 255, (glare_strength * 80.0) as u8),
                );
            }
        }
