...
impl Spotlight {
    /// Create a new spotlight with default settings
    pub fn new() -> Self {
        Self {
    ...
    pub fn show<R>(
        ...
        let color = self.color.unwrap_or_else(|| {
            Color32::from_rgba_unmultiplied(primary.r(), primary.g(), primary.b(), 40)
        });
        ...
        if should_draw {
            ...
            self.draw_spotlight(ui, rect, color);
    ...
    fn draw_spotlight(&self, ui: &mut Ui, _rect: Rect, color: Color32) {
        let painter = ui.painter();
        let center = self.current_pos;

        // Use unified GlowEffect for radial glow
        let glow = GlowEffect::new(
            GlowConfig::new(color)
                .layers(self.gradient_steps)
                .expansion(0.0) // radial glow doesn't need expansion
                .intensity(self.intensity),
        );

        glow.render_radial(&painter, center, self.radius);
    }

    /// Alternative rendering using mesh for potentially better performance
    #[allow(dead_code)]
    fn draw_spotlight_mesh(&self, ui: &mut Ui, _rect: Rect, color: Color32) {
        ...
        let shapes: Vec<Shape> = (0..self.gradient_steps)
            .map(|i| {
                ...
                let base_alpha = (color.a() as f32 * self.intensity) as u8;
...
impl MultiSpotlight {
    ...
    pub fn show<R>(
        ...
        for spotlight in &mut self.spotlights {
            let _ = spotlight.show(ui, |_ui| {
