...
impl MeteorShower {
    /// Create a new meteor shower with default settings
    pub fn new() -> Self {
        Self {
            meteors: Vec::new(),
            spawn_timer: 0.0,
    ...
    fn spawn_meteor(&mut self, bounds: Rect, color: Color32) {
        ...
        self.meteors
            .push(Meteor::new(start, end, speed, color, tail_length, 2.5));
    }
    ...
    pub fn show(&mut self, ui: &mut Ui) -> Response {
        ...
        if self.spawn_timer >= self.spawn_rate {
            self.spawn_meteor(bounds, color);
...
mod tests {
    ...
    fn test_meteor_config() {
        ...
        assert_eq!(shower.width, Some(800.0));
        assert_eq!(shower.height, Some(600.0));
