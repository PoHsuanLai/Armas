...
impl Tooltip {
    ...
    ///
    /// Returns the tooltip area's response
    pub fn show(&mut self, ui: &mut Ui, target_response: &Response) -> Response {
        let theme = ui.ctx().armas_theme();
        self.show_impl(ui, &theme, target_response)
    }

    fn show_impl(&mut self, ui: &mut Ui, theme: &crate::Theme, target_response: &Response) -> Response {
        ...
        if is_hovered {
            ...
            // Allocate an invisible rect and return its response
            let (_, response) = ui.allocate_exact_size(Vec2::ZERO, egui::Sense::hover());
            return response;
        }
...
pub fn tooltip_with(
    ...
    tooltip.show_impl(ui, &theme, response);
}

/// Tooltip bound to a target response
pub struct TooltipBound<'a> {
    tooltip: Tooltip,
    target: &'a Response,
}

impl Tooltip {
    /// Bind to a target response
    pub fn bind(self, target: &Response) -> TooltipBound<'_> {
        TooltipBound {
            tooltip: self,
            target,
        }
...
impl<'a> crate::traits::ArmasViewMut for TooltipBound<'a> {
    type Output = Response;

    fn ui(&mut self, ui: &mut egui::Ui) -> Response {
        let theme = ui.ctx().armas_theme();
        self.tooltip.show_impl(ui, &theme, self.target)
    }
}

impl<'a> ArmasModifiers for TooltipBound<'a> {}
