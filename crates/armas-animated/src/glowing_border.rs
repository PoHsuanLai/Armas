...
impl GlowingBorder {
    ...
    pub fn show<R>(
        ...
        let glow_color = if self.glow_color == Color32::PLACEHOLDER {
            theme.primary()
        } else {
            self.glow_color
        };
        ...
        let pulse_multiplier = if self.pulse {
            (time * self.pulse_speed * 2.0).sin().mul_add(0.4, 0.6)
        } else {
            1.0
        };
        ...
        if ui.is_rect_visible(rect) {
            ...
            let content_rect = rect.shrink(theme.spacing.md); // Padding
