        ...
        if ui.is_rect_visible(rect) {
            ...
            let (bg_color, text_color, border_color, draw_shadow) = if !enabled {
                ...
                match variant {
                    ButtonVariant::Filled => {
                        let hover_bg = blend_colors(theme.primary(), theme.hover(), 0.2);
                        (hover_bg, theme.on_surface(), theme.primary(), false)
                    }
                    ButtonVariant::FilledTonal => {
                        let hover_bg = blend_colors(theme.secondary(), theme.hover(), 0.15);
                ...
                match variant {
                    ...
                    ButtonVariant::Speaker => {
                        // Modern speaker button: smooth matte plastic
                        (
            ...
            if variant == ButtonVariant::Speaker {
                ...
                if is_pressed {
                    ...
                    let pressed_bg = Color32::from_rgb(36, 36, 36);
                    ...
                    // Normal/Hover: Subtle gradient (top to bottom, very soft)
                    let top_color = if response.hovered() {
                        Color32::from_rgb(46, 46, 46)
                    } else {
                        Color32::from_rgb(42, 42, 42)
                    };
                    let bottom_color = if response.hovered() {
...
fn blend_colors(base: Color32, overlay: Color32, factor: f32) -> Color32 {
    ...
    Color32::from_rgb(
        (base.r() as f32 * (1.0 - factor) + overlay.r() as f32 * factor) as u8,
        (base.g() as f32 * (1.0 - factor) + overlay.g() as f32 * factor) as u8,
        (base.b() as f32 * (1.0 - factor) + overlay.b() as f32 * factor) as u8,
    )
}
